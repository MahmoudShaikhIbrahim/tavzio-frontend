...
// all, since text-ivory already handles that itself. font-wordmark
// (Manrope, extrabold) is the actual font specified on the real
// business card, replacing the site's default Inter specifically here -
// a real font can't reproduce the card's hand-built custom letterform
// paths exactly, but it's the closest real, selectable, accessible text
// gets, and it's the literal typeface off the card rather than a guess.
//
...
export default function Logo({ className = '', size = 'md' }: { className?: string; size?: keyof typeof SIZES }) {
  return (
    <span className={`inline-flex items-center gap-2 font-wordmark font-extrabold uppercase tracking-wide text-ivory ${SIZES[size]} ${className}`}>
