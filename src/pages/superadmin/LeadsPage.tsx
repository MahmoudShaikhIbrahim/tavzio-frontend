...
import { listLeads, markLeadConverted } from '../../lib/authApi';
        ...
        {visible.map((lead) => (
          <div key={lead.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-ink-line px-4 py-3">
            ...
            {lead.converted ? (
              ...
              <button onClick={() => handleConvert(lead)} className="rounded-lg bg-brass px-3.5 py-1.5 text-sm font-medium text-ink hover:opacity-90">
                Convert to client
              </button>
            )}
          </div>
