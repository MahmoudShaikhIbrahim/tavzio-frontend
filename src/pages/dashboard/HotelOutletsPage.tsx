...
export default function HotelOutletsPage() {
  ...
  async function handleDelete(outlet: HotelOutlet) {
    if (!confirm(`Delete "${outlet.name}"? Guests will no longer be able to order from it.`)) return;
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="font-display text-3xl text-ivory">F&B Outlets & Services</h1>
        <p className="mt-1 text-base text-ivory-dim">
          Room Service, bars, the pool, breakfast - each is a separate outlet guests order from, but all draw
          from the same menu items you manage under Menu Management. Add an item to multiple outlets at once,
          each with its own optional price.
        </p>
      </div>
      ...
      <Section title="Outlets" action={
        ...
        {showAdd && (
          <form onSubmit={handleAdd} className="flex flex-wrap items-end gap-3 rounded-lg border border-ink-line p-4">
            <Field label="Name"><input required value={name} onChange={(e) => setName(e.target.value)} placeholder="Rooftop Bar" className={inputClass} /></Field>
            <Field label="Type">
            ...
            <Field label="Location"><input value={location} onChange={(e) => setLocation(e.target.value)} placeholder="Ground Floor" className={inputClass} /></Field>
            <Field label="Hours"><input value={openingHours} onChange={(e) => setOpeningHours(e.target.value)} placeholder="17:00–02:00" className={inputClass} /></Field>
            <div className="self-end"><PrimaryButton disabled={saving}>{saving ? 'Adding...' : 'Add'}</PrimaryButton></div>
          </form>
        )}
        ...
        <div className="space-y-3">
          {outlets.map((o) => (
            <div key={o.id} className="rounded-lg border border-ink-line p-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  <p className="text-base text-ivory">{o.name} <span className="text-xs uppercase text-brass">{OUTLET_TYPES.find((t) => t.value === o.outlet_type)?.label}</span></p>
          ...
          {outlets.length === 0 && <p className="text-ivory-dim">No outlets yet - add one above (e.g. "Room Service", "Pool Bar").</p>}
        </div>
      </Section>
    </div>
  );
...
function OutletItemPicker({ businessId, outlet, menuItems, onSaved }: {
  ...
  return (
    <div className="mt-3 space-y-3 rounded-lg border border-ink-line bg-ink-soft p-3">
      <p className="text-sm text-ivory-dim">Check every item this outlet should offer.</p>
