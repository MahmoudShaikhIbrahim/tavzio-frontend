...
function DashboardLayoutInner() {
  ...
  // time Orders was opened, not just once - "make it a normal button
  // like any other button"). The real problem was WHERE this lived:
  // fetched fresh inside OrdersPage itself, which unmounts and re-runs
  // its own effects every time someone navigates away and back, so the
  // brief "don't know yet" gap reopened on every single visit. Fetched
  // once here instead, at the shell level that stays mounted for the
  // whole session (exactly like refetchFeatures above), so by the time
  // any page asks for it, the real answer is already sitting there -
  // no flicker, because there's no per-visit fetch left to flicker.
